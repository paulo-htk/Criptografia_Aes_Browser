 ...
 */
export class UIManager implements MessageManager, TooltipManager {
     ...
     */
    private setupTooltipEvents(element: HTMLElement): void {
        ...
        (element as any)._tooltipEvents = { showTooltip, hideTooltip };
    }
     ...
     */
    public showTooltip(element: Element, content: string, position?: string): void {
     ...
     */
    private positionTooltip(element: Element, preferredPosition: string): void {
    ...
    public destroyTooltips(): void {
        ...
        document.querySelectorAll('[data-title]').forEach(element => {
            const events = (element as any)._tooltipEvents;
            if (events) {
                ...
                delete (element as any)._tooltipEvents;
            }
        });
    }
     ...
     */
    private initializeContentEditableElements(): void {
     ...
     */
    private setupContentEditablePlaceholder(element: HTMLElement): void {
     ...
     * @param element Elemento contenteditable
     */
    private setupContentEditableValidation(element: HTMLElement): void {
    ...
    private findTextNodeAtPosition(element: HTMLElement, position: number): { node: Text; offset: number } | null {
        ...
        if (walker.currentNode) {
            return {
                ...
                offset: (walker.currentNode as Text).textContent?.length || 0
            };
        }
    ...
    private calculateMessageDuration(message: string): number {
        ...
        return Math.max(
            this.messageConfig.baseDuration,
            this.messageConfig.baseDuration + (wordCount * this.messageConfig.durationFactor)
        );
    }
     ...
     */
    public addMessage(message: string, type: MessageType = 'info'): string {
     ...
     */
    public getInputValues(): { [key: string]: string } {
        return {
            keyHex: (this.elements.aesKey as HTMLInputElement).value.trim(),
            ivHex: (this.elements.iv as HTMLInputElement).value.trim(),
     ...
     */
    public updateOutputValues(values: OutputValues): void {
            ...
            switch (key) {
                case 'aesKey':
                    if (this.elements.aesKey) {
                        (this.elements.aesKey as HTMLInputElement).value = value;
                    }
                    break;
                case 'iv':
                    if (this.elements.iv) {
                        (this.elements.iv as HTMLInputElement).value = value;
                    }
                    break;
    ...
    public clearAllInputs(): void {
        (this.elements.aesKey as HTMLInputElement).value = '';
        (this.elements.iv as HTMLInputElement).value = '';
     ...
     */
    public setElementEnabled(elementKey: keyof DOMElements, enabled: boolean): void {
        const element = this.elements[elementKey];
        if (element) {
     ...
     */
    public getElements(): Readonly<DOMElements> {
